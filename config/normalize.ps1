...
$tv_in = "/media/tv-in/" 
$tv_out = "/media/tv-out/"
$film_in = "/media/film-in/"
$film_out = "/media/film-out/"
Clear-Host


Write-Host $File
Start-Sleep -Seconds .5
$FileOG = Get-Childitem -Path $File -ErrorAction Stop

###################################

 
If (-Not (Test-Path -Path $file )) {
    Write-Output "File does not exist, quiting"
    Exit
    }
 
    Write-Host "file tested"
    Start-Sleep -Seconds .5
...
#### Move folder ####
if ($FileOG.FullName.Split('/')[2] -eq "film-in") {
    Move-Item -Path ($film_in + $FileOG.Directory.Name) -Destination ($film_out + $FileOG.Directory.Name)
}
if ($FileOG.FullName.Split('/')[2] -eq "tv-in") {
    Move-Item -Path ($tv_in + $FileOG.Directory.Name) -Destination ($tv_out + $FileOG.Directory.Name)
}
